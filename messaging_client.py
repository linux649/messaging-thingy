...
from messaging_modules.status import START
...
last_update_time = time.time() - HOUR_S
sent_sync = 0
...
def write_new_config_file(config_file: str, id:uuid.UUID=uuid.uuid4()):
        ...
        while not close_encrypted_socket.is_set():
            ...
            match packet_status:
                ...
                case status.REPLY_USER_INFO:
                    ...
                    direct_channel_select.delete(START, tkinter.END)
                    room_channel_select.delete(START, tkinter.END)
                    ...
                    #in the following, \u00a0 is used so the change_to<room_type> events can read the channel id, without having to rely on an extra list.
                        direct_channel_select.insert(tkinter.END, f'{other_name} ({other_id}){SPACE}{direct}')
                    for room_id, name in rooms.items():
                        room_channel_select.insert(tkinter.END, name + SPACE + room_id)
                ...
                case status.REPLY_MESSAGES:
                    ...
                    if not can_acquire:
                        ...
                        current_type, current_id = current_room.get().split(SPACE)
...
def initialise_encrypted_connection(common_name:str, host:str, tls_port:int):
    ...
    encrypted_conn = socket.create_connection((host, tls_port), SOCKET_RECIEVE_TIMEOUT)
...
def cycle_display_mode():
    ...
    if display_mode == DISPLAY_MAIN_MENU:
        connect_menu.grid(row=0, column=0)
...
def on_connect_listbox_select(event):
    ...
    try:
        ...
        ip_addr_entry.delete(START, tkinter.END)
        ip_addr_entry.insert(START, ip_addr)

        tcp_port_entry.delete(START, tkinter.END)
        tcp_port_entry.insert(START, tcp_port)
...
def on_send_message_event(*event):
    ...
    if current_room:
        with current_room_lock:
            channel_type, channel_id = current_room.get().split(SPACE)
            ...
            messages_entry.delete(START, tkinter.END)
...
def change_to_direct(*event):
    ...
    try: #_tkinter.TclError is raised when the client hasn't yet recieved the info from the server yet
        ...
        direct_id = selection.split(SPACE, maxsplit=1)[1]
    ...
    if functions.check_uuid_valid(direct_id):
        ...
        if can_acquire:
            current_room.set(status.CHANNEL_TYPE_DIRECT+SPACE+direct_id)
...
def change_to_room(*event):
    ...
    try:#_tkinter.TclError is raised when the client hasn't yet recieved the info from the server yet
        ...
        room_id = selection.split(SPACE,maxsplit=1)[1]
    ...
    if functions.check_uuid_valid(room_id):
        can_acquire = current_room_lock.acquire(blocking=False)
        if can_acquire:
            current_room.set(status.CHANNEL_TYPE_ROOM+SPACE+room_id)
            reset_message_display()
            current_room_lock.release()
...
def on_set_username():
    ...
    username = tkinter.simpledialog.askstring('Ser Username', 'Enter your new username: (long usernames will be shortened)')
    if len(username) > status.USERNAME_MAX_LEN:
...
if __name__ == '__main__':
    ...
    menu_exit_button = tkinter.ttk.Button(main_menu, text='Exit', command=on_close)
    ...
    network_discovery_frame.grid(row=0, column=0)
    ...
    connect_info_frame.grid(row=1, column=0)
    ...
    message_entry_frame.grid(row=1, column=0, rowspan=2, sticky=tkinter.EW)
